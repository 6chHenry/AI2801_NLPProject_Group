"""
下载并缓存模型与数据集到本地 ./cache 目录（小组实验：PG-19 20 books）
"""
import os
import json
...
# ── 3. pg-19（test split 前 20 个样本）──────────────────────────────────────────
print("=" * 60)
print("[3/3] 下载 pg-19 数据集（streaming，取 test 前 20 条）...")
...
pg19_path = os.path.join(CACHE_DIR, "pg19_test_20samples.json")
with open(pg19_path, "w", encoding="utf-8") as f:
    json.dump(samples, f, ensure_ascii=False, indent=2)
print(f"  已保存至 {pg19_path}")

legacy_path = os.path.join(CACHE_DIR, "pg19_test_5samples.json")
with open(legacy_path, "w", encoding="utf-8") as f:
    json.dump(samples[:5], f, ensure_ascii=False, indent=2)
print(f"  兼容文件: {legacy_path}")
